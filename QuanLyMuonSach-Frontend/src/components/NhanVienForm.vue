  ...
  <div class="modal-backdrop">
    <div class="modal d-block" tabindex="-1">
      <div class="modal-dialog">
        <!-- tắt autocomplete trình duyệt trên form -->
            ...
            <div class="modal-body">
              <div class="row g-3">
                ...
                <div class="col-md-6">
                  ...
                  <!-- đặt autocomplete phù hợp để trình duyệt không dùng giá trị username -->
...
const props = defineProps({
  initial: { type: Object, default: null }, // object chứa dữ liệu khi edit
  // nếu parent không reset initial, ta dùng key để reset form khi cần
...
// key để buộc recreate form nếu parent thay đổi modalId (parent có thể increment khi mở modal mới)
const modalKey = computed(() => `modal-${props.modalId}-${!!props.initial}`);

const isEdit = computed(() => !!props.initial?._id);

// khi props.initial thay đổi thì populate form (sửa mapping lỗi và đảm bảo reset đúng)
watch(
  () => props.initial,
  (val) => {
    if (val) {
      ...
      // reset khi initial null/undefined
      form.value = {
        msnv: "",
        hoTenNV: "",
        password: "",
...
async function submit() {
  try {
    ...
    let saved;
    if (isEdit.value) {
      saved = await nhanVienService.updateNhanVien(props.initial._id, payload);
    } else {
      // đảm bảo create gửi password (template đã required khi !isEdit)
      saved = await nhanVienService.createNhanVien(payload);
    }
